...
class snmp(unittest.TestCase):
    ...
    def test_config_community_withIpAddr(self):
        """Test snmp configure with IPaddr sub-command (community ipAddres).
        ...
        ipaddress_values = str("10.10.10.1")+","+str("10.10.10.2")+","+str("10.10.10.3")+","+ \
                           str("10.10.10.4")
        ...
        pipe = subprocess.Popen("%s/bin/comea snmp configure community %s ipAddress %s"
            % (self._comea_root, community_values, ipaddress_values), shell=True, stderr=subprocess.PIPE,stdout=subprocess.PIPE)
    ...
    def test_config_community_withoutIpAddr(self):
        ...
        # configure community values
        pipe = subprocess.Popen("%s/bin/comea snmp configure community %s"
            % (self._comea_root, community_values), shell=True, stderr=subprocess.PIPE,stdout=subprocess.PIPE)
    ...
    def test_config_trapsess(self):
        """Test trapsess configuration written to the conf file. Expect clean outputs and zero return code."""
